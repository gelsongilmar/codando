...
{
    internal class ArquivoProcedureDelete : Arquivo
    {
        private EntidadeGerada _entidade;

        public ArquivoProcedureDelete(EntidadeGerada entidade)
        {
            this._entidade = entidade;
            this.UsarArquivoSeparadoParaRegerar = false;
            this.Nome = "usp_" + this._entidade.Nome + "_Delete";
            this.Extensao = ".sql";
        }
        ...
        public override string GetConteudoRegerado()
        {
            ...
            _str.AppendLine(" AS ");
            _str.AppendLine("    DELETE FROM " + this._entidade.Nome);
            _str.Append("    WHERE ");
